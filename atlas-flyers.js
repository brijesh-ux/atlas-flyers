...
var CAT_SIDEBAR_HIDE = "#searchspring-sidebar,#facetedSearch,.facetedSearch,.ss__sidebar{display:none!important}";
...
function ssFParams(){
  var brands=[],price=null,stock=false;
  try{
  ...
  return {brands:brands,price:price,stock:stock,any:brands.length>0||!!price||stock};
}
...
function ssBuildFilterBar(facets,hostEl){
  ...
  if(f.any)chip('Clear all',function(sp){sp.delete('fbrand');sp.delete('fprice');sp.delete('fstock');},'fp-fb-clear');
  ...
  try{
    ...
    (async function(){
      try{
        ...
        if(facets)ssBuildFilterBar(facets);
      }catch(e){}
    })();
