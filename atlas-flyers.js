...
function catBuildFilterBar(){
  // REFINE BY bar built from BigCommerce's server-rendered facet data (the old
  // bar was SearchSpring UI and died with its takeover; the theme parks the
  // native block off-screen). Clicking navigates BigCommerce's own facet URLs.
  if(document.getElementById('fp-filter-bar'))return;
  var fs=document.querySelector('#facetedSearch');
  ...
  [].slice.call(fs.querySelectorAll('.accordion-content')).forEach(function(content){
    ...
    chips.forEach(function(c){
      ...
      chip.className='fp-fb-chip';chip.href=c.h;chip.textContent=c.t;
  ...
  if(!groups&&!bar.querySelector('.fp-fb-chip'))return;
...
async function initCategoryTiles(rerun){
  ...
  try{
    ...
    var newestMode=newestModeEarly;
