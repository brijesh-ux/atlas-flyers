...
var CAT_SIDEBAR_HIDE = "#searchspring-sidebar,#facetedSearch,.facetedSearch,.ss__sidebar,.floating-filter-btn,.floating-mobile-filter-btn,.ssc-slideout-button,form.actionBar,.breadcrumbs{display:none!important}";
...
// collapse the theme's .atlas-filter-bar ONLY when nothing visible is left
// inside it (its Snap sidebar is hidden on taken-over pages -> empty bordered
// husk under the breadcrumbs); categories using it for subcategory links keep it
function fpCollapseEmptyBar(){
  [].slice.call(document.querySelectorAll('.atlas-filter-bar')).forEach(function(b){
    var vis=[].slice.call(b.querySelectorAll('a,button')).some(function(el){
      var r=el.getBoundingClientRect();
      return r.width>0&&r.height>0;
    });
    if(!vis)b.style.setProperty('display','none','important');
  });
}
...
async function initCategoryTiles(rerun){
  ...
  try{
    ...
    catWidenContainer();
    fpCollapseEmptyBar();
    setTimeout(fpCollapseEmptyBar,2000);
    setTimeout(fpCollapseEmptyBar,5000);
...
(function(){
  ...
  hideSt.textContent='#searchspring-content{display:none!important}'
    ...
    +'.floating-filter-btn,.floating-mobile-filter-btn,.ssc-slideout-button,.breadcrumbs{display:none!important}';
