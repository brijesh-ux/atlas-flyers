...
(function(){
  ...
  // v78: the theme DISABLES the add button while an add-to-cart request is in
  // flight (and swaps its text to data-wait-message "Adding to cart..."), so a
  // bare `disabled` check flashes NOTIFY ME for the duration of every add.
  // Distrust `disabled` while in flight, for 6s after a real add click, and
  // never flip on a single observation (see pdpSync confirm-twice).
  var fpAddClickAt=0,fpUnFirst=0;
  document.addEventListener('click',function(e){
    if(e.target&&e.target.closest&&e.target.closest('[id="form-action-addToCart"]'))fpAddClickAt=Date.now();
  },true);
  function pdpUnavailable(){
    ...
    var graced=Date.now()-fpAddClickAt<6000;
    var abs=pdpAddButtons();
    for(var i=0;i<abs.length;i++){
      var t=((abs[i].textContent||abs[i].value)||'').trim();
      if(/out of stock|unavailable|sold out/i.test(t))return true;
      var inFlight=/adding/i.test(t)||t===((abs[i].getAttribute('data-wait-message')||'').trim());
      if(abs[i].disabled&&!inFlight&&!graced)return true;
    }
    return false;
  }
  function pdpSync(){
    try{
      ...
      if(un&&!document.body.classList.contains('fp-pdp-unavail')){
        ...
        else if(nowT-fpUnFirst<550){un=false;}
      }else if(!un){fpUnFirst=0;}
...
var CAT_SIDEBAR_HIDE = "#searchspring-sidebar,#facetedSearch,.facetedSearch,.ss__sidebar,.floating-filter-btn,.floating-mobile-filter-btn,.ssc-slideout-button,form.actionBar,.breadcrumbs,.atlas-filter-bar{display:none!important}";
